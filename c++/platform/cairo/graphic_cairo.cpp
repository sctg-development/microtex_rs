...
void Font_cairo::loadFont(const string& file) {
  ...
  // store a human-readable name (font family if available)
  if (family)
    _name = reinterpret_cast<const char*>(family);
  else
    _name = file;
  ...
  _cairoFtFaces.insert(
    std::pair<string, cairopp::CairoFontFacePtr>(
      file,
...
void Graphics2D_cairo::drawGlyph(u16 glyph, float x, float y) {
  if (_font && _font->getCairoFontFace()) {
...
void Graphics2D_cairo::drawTextLiteral(const std::string& text, float x, float y) {
  // Use Pango to render UTF-8 literal text at the specified position.
  PangoLayout* layout = pango_cairo_create_layout(_context);
  PangoFontDescription* fd = pango_font_description_new();

  // prefer the human-readable font name when available
  const char* fam = "Serif";
  if (_font) {
    const auto& nm = _font->name();
    ...
    if (!nm.empty() && is_printable(nm)) fam = nm.c_str();
  }

  pango_font_description_set_family(fd, fam);
  pango_font_description_set_absolute_size(fd, _fontSize * PANGO_SCALE);
  pango_layout_set_text(layout, text.c_str(), text.length());
  pango_layout_set_font_description(layout, fd);
  pango_font_description_free(fd);

  // compute ascent and position correctly
  int baseline = pango_layout_get_baseline(layout);
  const float ascent = (float)baseline / (float)PANGO_SCALE;

  cairo_save(_context);
  cairo_translate(_context, x, y - ascent);
  pango_cairo_show_layout(_context, layout);
