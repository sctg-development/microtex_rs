...
public:
  ...
  // Draw a literal UTF-8 text string at the given position (used as a fallback
  // when glyph IDs are not available or when the caller emits literal text).
  void drawTextLiteral(const std::string& s, float x, float y) override;
};
