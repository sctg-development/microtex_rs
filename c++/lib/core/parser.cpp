...
string Parser::getGroup(char open, char close)
{
  ...
  if (_pos < _len && ch == open)
  {
    ...
    if (group != 0)
      return _latex.substr(spos + 1, _pos - spos - 1);
...
string Parser::getCmd()
{
  ...
  if (com == "cr" && _pos < _len && _latex[_pos] == ' ')
    _pos++;
  return com;
}
...
sptr<Atom> Parser::processEscape()
{
  ...
  // Avoid recursive construction that starts with "\\mathtt" (can re-enter processEscape on 'mathtt').
  // Build a formula for the literal backslash+cmd and apply monospaced font without introducing another "\\mattt" escape.
  auto rm = sptrOf<FontStyleAtom>(
      FontStyle::tt,
      false,
      Formula("{\\backslash}" + cmd)._root);
