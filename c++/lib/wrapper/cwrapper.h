...
extern "C"
{
  ...
  /**
   * Render a previously parsed render to an SVG document using Cairo.
   *
   * Returns a malloc'd buffer containing the SVG bytes and writes its length
   * to @out_len. Caller must free the returned buffer by calling
   * [microtex_free_buffer()].
   */
