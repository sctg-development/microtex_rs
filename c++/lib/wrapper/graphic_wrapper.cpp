...
void Graphics2D_wrapper::setFont(const sptr<Font>& font) {
  ...
  _font = font;
#ifdef HAVE_CAIRO
  // If the platform has Cairo active the font might be a Font_cairo; prefer
  // using its human-readable name when available.
  ...
  if (auto fw = std::dynamic_pointer_cast<Font_wrapper>(_font)) {
    const auto& nm = fw->name();
    if (is_printable(nm)) {
      _cmds.put((u8)3, nm.c_str());
    } else {
      _cmds.put((u8)3, "");
    }
  } else {
...
#else
  // No Cairo: always emit empty font name
  _cmds.put((u8)3, "");
#endif  // HAVE_CAIRO
}
...
void Graphics2D_wrapper::drawTextLiteral(const std::string& text, float x, float y) {
  _cmds.put((u8)23, text.c_str(), x, y);
}
