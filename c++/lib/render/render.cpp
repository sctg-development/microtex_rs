...
int Render::getHeight() const {
  auto box = _data->root;
  // Use ceil to ensure no content is truncated when converting to integer
  return (int)std::ceil(box->vlen() * _data->fixedScale);
}

int Render::getDepth() const {
  // Use ceil to ensure no content is truncated when converting to integer
  return (int)std::ceil(_data->root->_depth * _data->fixedScale);
}

int Render::getWidth() const {
  // Use ceil to ensure no content is truncated when converting to integer
  return (int)std::ceil(_data->root->_width * _data->fixedScale);
}
