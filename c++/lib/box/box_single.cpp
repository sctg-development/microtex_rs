...
void _drawWithFont(const Char& chr, Graphics2D& g2, float x, float y) {
  ...
  // If the target Graphics2D is the wrapper (we are producing drawing data for
  // headless consumption), export literal unicode characters instead of
  // opaque glyph IDs so the Rust fallback renderer can display readable text.
#ifdef HAVE_CWRAPPER
  if (dynamic_cast<Graphics2D_wrapper*>(&g2)) {
    std::string s;
    appendToUtf8(s, chr.mappedCode);
    fprintf(stderr, "detected wrapper target, emitting literal: '%s'\n", s.c_str());
    if (chr.isValid()) {
      g2.drawTextLiteral(s, x, y);
      return;
    }
...
#ifdef HAVE_CWRAPPER
  // When the wrapper backend is active, export literal UTF-8 characters instead
  // of opaque glyph IDs so headless renderers can draw text.
  if (dynamic_cast<PlatformFactory_wrapper*>(factory)) {
    std::string s;
    appendToUtf8(s, chr.mappedCode);
    if (chr.isValid()) {
      g2.drawTextLiteral(s, x, y);
...
#ifdef HAVE_CAIRO
  // If the Cairo backend is active but the font cannot produce a valid
  // cairo_font_face (e.g., CLM data is not a system TTF), fall back to
  // emitting literal UTF-8 so native rendering via Pango/cairo can draw it.
  if (auto fc = std::dynamic_pointer_cast<Font_cairo>(font)) {
    if (fc->getCairoFontFace() == nullptr) {
      std::string s;
      appendToUtf8(s, chr.mappedCode);
      if (chr.isValid()) {
        g2.drawTextLiteral(s, x, y);
...
#endif
  // If the underlying font fontFile appears to be a CLM (bundled math data)
  // then glyph IDs are internal and unlikely to map to system font glyph
  // indexes — prefer rendering a literal UTF-8 character via Pango/cairo.
  if (auto of = chr.otfFont()) {
    const auto ff = of->fontFile;
    if (ff.find(".clm") != std::string::npos || ff.find("clm") != std::string::npos) {
      std::string s;
      appendToUtf8(s, chr.mappedCode);
      if (chr.isValid()) {
        g2.drawTextLiteral(s, x, y);
