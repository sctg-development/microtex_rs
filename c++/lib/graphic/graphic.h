...
public:
  ...
  /**
   * Draw a literal UTF-8 text string at (x, y). This is primarily used by the
   * wrapper backend to export textual characters (Unicode) instead of glyph IDs.
   * Default implementation is a no-op, platforms may override for more fidelity.
   */
  virtual void drawTextLiteral(const std::string& s, float x, float y) { (void)s; (void)x; (void)y; }
